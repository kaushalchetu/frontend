...
export const sic14GraphSelector = createSelector(
    getUCData,
    getUDData,
    (ucData, udData) => {
        const ci = []
        ...
        ucData && ucData.forEach((value, index) => {
            ...
            if (ciData.length && ciData[3] > ciData[1]) {
                for (let i = ciData[1]; i < ciData[3]; i += 1) {
                    ci.push({
                        name: ciData[0] + "+" + i,
                        color: value.color,
            ...
            if (cdData.length && cdData[3] > cdData[1]) {
                for (let i = cdData[1]; i < cdData[3]; i += 1) {
                    cd.push({
                        name: cdData[0] + "+" + i,
                        color: cdColor
                    })
        ...
        return { ci, cd, chunks: chunkArray(totalIndex, 30) }
    }
)